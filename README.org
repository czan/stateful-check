...
- ~:commands~ is a map of command specifications (see below), which
  are the operations which ~stateful-check~ knows how to perform. The
  keys to this map are opaque to ~stateful-check~, but are used in the
  ~:generate-commands~ function (and must be printable).
